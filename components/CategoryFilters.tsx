...
export const CategoryFilters: React.FC<CategoryFiltersProps> = ({ activeCategory, onCategoryChange }) => {
  ...
  const allButtonClasses = `px-3 py-1.5 text-sm font-semibold border rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-transparent ${
    activeCategory === null
      ? 'bg-cyan-500/90 text-white ring-cyan-400 border-cyan-500/80'
      : 'border-slate-300 text-slate-500 bg-slate-200/50 hover:bg-slate-300/50 hover:border-slate-400 transform hover:-translate-y-0.5'
  }`;
  
  return (
    <div className="bg-slate-100/50 backdrop-blur-sm p-3 rounded-xl border border-slate-200">
